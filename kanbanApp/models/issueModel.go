...
func (issue Issue) Migrate() {
	...
	db.AutoMigrate(&issue) //This block creates a table with this name on the database.
}
...
func GetIssue(c *gin.Context) {
	...
	var issues []Issue
	db.Preload("Comments").Find(&issues) //Also loads comment records related the issue
	c.JSON(http.StatusOK, issues)
}
func (issue Issue) Get(where ...interface{}) Issue { //Get a spesific issue
	db := GetDB()
