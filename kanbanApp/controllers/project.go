...
func (project Project) WorkSpaceDelete(c *gin.Context) {
	...
	// Veritabanından mevcut görevi al
...
// Sidebar workspace form Proje ekleme
func (project Project) ProjectAdd(c *gin.Context) {
	name := c.PostForm("name")
	description := c.PostForm("description")
	workspaceIDStr := c.PostForm("workspace_id")

	// Seçilen checkbox'ların e-posta adreslerini al
	selectedCheckboxesEmails := c.PostFormArray("checkboxes")

	// workspaceID'yi uint'e dönüştür
	...
	// Proje ID'sini al
	projectID := newProject.ID // Projeyi ekledikten sonra yeni oluşturulan ID'yi kullan
	...
	addUserRequest := struct {
		...
		ProjectID: projectID, // Doğru proje ID'si
		Emails:    selectedCheckboxesEmails,
	}

	// AddUserToProjectByEmail fonksiyonunu çağır
...
func (project Project) ProjectUserAdd(c *gin.Context) {
	...
	addUserRequest := struct {
		...
		ProjectID: uint(projectID), // Convert to uint
		Emails:    selectedCheckboxesEmails,
		Role:      "member",
	}
...
// Sidebar procejt form Proje güncelleme
